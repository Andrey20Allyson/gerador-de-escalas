...
const CONFIG_PATH = path.resolve(process.cwd(), `auto-schedule.config.yml`);
...
function recursiveAssign<T>(
  partialValue: RecursivePartial<T> | undefined | null,
  defaultValue: T,
): T {
  ...
  for (const key in defaultValue) {
    object[key] = recursiveAssign<T[Extract<keyof T, string>]>(
      partialValue[key],
      defaultValue[key],
    );
  }
...
export function createAutoScheduleConfig(
  factory: (config: AutoScheduleConfig) => RecursivePartial<AutoScheduleConfig>,
): AutoScheduleConfig {
