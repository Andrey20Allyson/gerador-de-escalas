...
export type ModalProps<T extends ModalLike> =
  T extends ModalLike<infer P> ? P : never;
...
export function createModalContext<M extends ModalLike<any>>(Modal: M) {
  ...
  const ModalContext = createContext<InternModalContext<ThisModalProps> | null>(
    null,
  );

  function ModalProvider(props: PropsWithChildren) {
    const [_props, setProps] = useState<ThisModalProps | EmpityProps>(
      EMPITY_PROPS,
    );
    ...
    return (
      <ModalContext.Provider
        value={{ props: _props, isOpen, close, open, isClosing, setProps }}
      >
        {props.children}
  ...
  function useModalContext() {
    ...
    if (!context)
      throw new Error(`Can't use ${Modal.name}Context outside a Provider!`);
