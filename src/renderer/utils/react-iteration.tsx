...
export type IterProps<T, P extends {} = {}> = Omit<P, EntryPropKey> & { [K in EntryPropKey]: T };
...
export type ElementListProps<T, P extends {}> = Exclude<keyof P, 'entry'> extends never
...
export function ElementList<T, P extends {}>(props: ElementListProps<T, P>) {
  const { Component, iter, reducer, communProps } = props as IterElementsOptions<T, P>;

  return <>{Array.from(iterElements({ Component, iter, communProps, reducer }))}</>;
}
...
export function* iterElements<T, P extends {}>(options: IterElementsOptions<T, P>): Iterable<React.JSX.Element> {
