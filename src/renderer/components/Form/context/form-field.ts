...
export class FormField implements Result.OnReusltError {
  ...
  handlerTo<E extends object>(
    attr: Exclude<
      AttrNameThatTypeExtends<E, FormFieldValue>,
      keyof EventTarget
    >,
  ): InputHandler<E> {
    return {
      ...
      handleRef: (ref) =>
        ref !== null && !this.exist() && this.set(ref[attr] as FormFieldValue),
    };
  }

  pipe<R extends FormFieldValue>(
    parser: ParserLike<FormFieldValue, R>,
  ): ValuePipe<R> {
