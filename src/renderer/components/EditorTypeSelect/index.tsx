...
import {
  ...
  tableEditorSelector,
} from "../../state/slices/table-editor";
...
export function EditorTypeSelect() {
  const table = useAppSelector((state) =>
    currentTableSelector(tableEditorSelector(state)),
  );
...
export function EditorTypeOption(props: EditorTypeOptionProps) {
  ...
  return (
    <StyledEditorTypeButton
      selected={route.is(actualRoute.name)}
...
export const StyledEditorTypeButton = styled.button<StyledEditorTypeButtonProps>`
  ...
  box-shadow:
    calc(var(--shadow-spread) * -1) calc(var(--shadow-spread) * 1) 0.1rem #0003,
    calc(var(--shadow-spread) * 1) calc(var(--shadow-spread) * -1) 0.1rem #fff4;
  ${buttonColorStyle}
