...
import {
  transformWorkers,
  transformWorkersOptionsSchema,
} from "./actions/transform-workers";
...
loadCommand({
  ...
  optionInfos: {
    date: OptionInfoBuilder.alias("d").describe(
      "the month of extra duty table",
    ),
...
loadCommand({
  ...
  optionInfos: {
    times: OptionInfoBuilder.alias("t").describe(
      "How much times the program with run the assign procedure",
    ),
    weight: OptionInfoBuilder.alias("w")
...
loadCommand({
  ...
  optionInfos: {
    input: OptionInfoBuilder.alias("i").describe(
      "Input file, a .json that implements a Array<WorkerRegistryInit>",
    ),
  },
  action: upload,
});
...
loadCommand({
  ...
  optionInfos: {
    source: OptionInfoBuilder.alias("s").describe(
      "Input source file, a .js exports by default a Transformer",
    ),
  },
