export class ExcelDate {
  ...
  static readonly AVERAGE_TROPICAL_MONTH =
    ExcelDate.AVERAGE_TROPICAL_YEAR / ExcelDate.MONTHS_IN_A_YEAR;
  ...
  static normalize(year: number, month: number, day: number) {
    return (
      (year - ExcelDate.FIRST_YEAR) * ExcelDate.AVERAGE_TROPICAL_YEAR +
  ...
  static parse(date: number): ExcelDate {
    ...
    const month = Math.floor(
      (date - year * ExcelDate.AVERAGE_TROPICAL_YEAR) /
        ExcelDate.AVERAGE_TROPICAL_MONTH,
    );
    const day =
      Math.floor(
        date -
          year * ExcelDate.AVERAGE_TROPICAL_YEAR -
          month * ExcelDate.AVERAGE_TROPICAL_MONTH,
      ) - 1;

    return new ExcelDate(year + ExcelDate.FIRST_YEAR, month, day);
  }
}
