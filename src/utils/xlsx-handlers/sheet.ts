...
export class SheetHandler {
  ...
  constructor(readonly sheet: XLSX.WorkSheet) {
  ...
  safeAt(collumn: string, line: number): ResultType<CellHandler> {
    ...
    const cell: XLSX.CellObject | XLSX.WSKeys =
      this.sheet[cellAddress] ?? this.createCellObject(cellAddress);
    if (!this.isCellObject(cell))
      return ResultError.create(`Unespected type of cell: ${cell}`);
  ...
  *iterLines(
    start = this.ref.start.line,
    end = this.ref.end.line + 1,
  ): Iterable<LineHander> {
