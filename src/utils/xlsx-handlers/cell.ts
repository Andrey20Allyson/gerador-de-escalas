...
export class CellHandler<V extends CellAnyType = CellAnyType> {
  static readonly CONSTRUCTORS_MAP: ReadonlyMap<Function, RunTimeCellTypes> =
    new Map<Function, RunTimeCellTypes>([
  ...
  static readonly TYPE_NAMES: ReadonlyMap<CellTypes, string> = new Map<
    CellTypes,
    string
  >([["empity", "undefined"]]);

  static readonly XLSX_TYPE_MAP: ReadonlyMap<
    RunTimeCellTypes,
    XLSX.ExcelDataType
  > = new Map<RunTimeCellTypes, XLSX.ExcelDataType>([
    ["boolean", "b"],
    ["string", "s"],
    ["empity", "z"],
    ["number", "n"],
  ...
  constructor(readonly cell: XLSX.CellObject) {
  ...
  static safeTypeOf(
    value: CellTypeMap[CellTypes],
  ): ResultType<RunTimeCellTypes> {
    if (value === undefined) return "empity";

    const constructor = value.constructor;

    return (
      this.CONSTRUCTORS_MAP.get(constructor) ??
      ResultError.create(`Unespected type '${constructor.name}'`)
    );
  }
  ...
  is<T extends CellTypes>(type: T): this is CellHandler<CellTypeMap[T]> {
    return (
      type === "any" ||
      type.includes(this._type) ||
      (this._type === "empity" && type.at(-1) === "?")
    );
  }

  safeAs<T extends CellTypes>(
    type: T,
  ): ResultType<CellHandler<CellTypeMap[T]>> {
    return this.is(type)
      ? this
      : ResultError.create(
          `Can't assign cell with type '${CellHandler.getTypeName(this._type)}' as a '${CellHandler.getTypeName(type)}'`,
        );
  }

  static typeAll<
    C extends readonly CellHandler[] | [],
    T extends readonly CellTypes[] | [],
  >(cells: C, types?: T) {
  ...
  >(
    ...
    -readonly [K in keyof C]: CellHandler<
      CellTypeMap[K extends keyof T ? T[K] : CellTypes]
    >;
  }> {
