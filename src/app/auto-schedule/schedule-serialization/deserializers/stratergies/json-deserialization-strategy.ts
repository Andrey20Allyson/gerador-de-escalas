import { DaysOfWork, ExtraDutyTable, ExtraDutyTableConfig, WorkerInfo, WorkLimit, WorkTime } from "../../../extra-duty-lib";
...
export class JsonDeserializationStratergy implements DeserializationStratergy {
  async execute(buffer: Buffer): Promise<ExtraDutyTable> {
    ...
    const workers = (json.workers as any[]).map(worker => this._objectIntoWorker(worker, table.month));
  ...
  protected _objectIntoWorker(object: any, month: Month): WorkerInfo {
    ...
    const daysOfWork = DaysOfWork.fromRestrictionArray(object.ordinaryDays, month.year, month.index);
    ...
    const worker = new WorkerInfo({
      ...
      daysOfWork,
      workTime: new WorkTime(object.ordinaryStartHour, object.ordinaryDurationHour),
  ...
  protected _applyScheduleObject(schedute: any[], table: ExtraDutyTable, workers: WorkerInfo[]): void {
    ...
    for (const [dayIndex, dutyIndex, workerId] of schedute) {
      ...
      if (worker == null) {
        throw new Error(`Invalid Data, worker with id '${workerId}' hasn't declarated`);
      }
