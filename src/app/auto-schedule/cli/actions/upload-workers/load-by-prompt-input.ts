...
import { WorkerRegistryGender, WorkerRegistryInit } from "../../../persistence/entities/worker-registry";
...
async function input(config: InputConfig) {
  ...
  const { [name]: value } = await prompt.get({ name, pattern, message, required: true, type: 'string' });
