...
import CryptorWithPassword, { CryptorWithPasswordDecryptError, CryptorWithPasswordDecryptErrorCause, CryptorWithPasswordInitError } from './cryptor-with-password';
...
describe(CryptorWithPassword, () => {
  test(`Constructor shold throw a ${CryptorWithPasswordInitError.name} if password is empity`, () => {
    expect(() => new CryptorWithPassword({ password: '' })).toThrow(CryptorWithPasswordInitError);
  });
  ...
  test(`Shold throw a ${CryptorWithPasswordDecryptError.name} with cause ${CWPDECause('probable-incorrect-password')} if password is incorrect`, async () => {
    ...
    await expect(decryptedBuffer).rejects.toBeInstanceOf(CryptorWithPasswordDecryptError).catch(error => {
      if (error instanceof CryptorWithPasswordDecryptError) {
        return expect(error.cause).toEqual('probable-incorrect-password' satisfies CryptorWithPasswordDecryptErrorCause);
      } else {
        expect.fail(`Error don't is instance of ${CryptorWithPasswordDecryptError.name}`);
      }
    });
  });
});