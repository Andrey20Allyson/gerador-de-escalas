...
export type IpcMapping<
  T extends IpcMapping.HandlerMap = IpcMapping.HandlerMap,
> = {
  thisArg?: unknown;
  map: T;
};

export interface IpcMappingFactory<
  T extends IpcMapping.HandlerMap = IpcMapping.HandlerMap,
> {
...
export namespace IpcMapping {
  ...
  export type HandlerFunctionReturnType = OptionalPromise<void | AppResponse<
    any,
    any
  >>;
  ...
  export function create<M extends HandlerMap>(
    map: M,
    thisArg?: unknown,
  ): IpcMapping<M> {
    return { thisArg, map };
...
export type IpcInvokerMapFromFactory<F extends IpcMappingFactory> =
  F extends IpcMappingFactory<infer M> ? IpcInvokerMap<M> : never;
