...
export const DEFAULT_MEDICAL_LICENSE_REGEXP =
  /LICENÇA MÉDICA \((DE (\d{2}\/\d{2}\/\d{2,4}) )?ATÉ (\d{2}\/\d{2}\/\d{2,4})\)/;
export const DEFAULT_PREMIUM_LICENSE_REGEXP =
  /LICENÇA PRÊMIO \((DE (\d{2}\/\d{2}\/\d{2,4}) )?ATÉ (\d{2}\/\d{2}\/\d{2,4})\)/;
...
export class LicenseIntervalParser {
  ...
  constructor(config: LicenseIntervalParserConfig = {}) {
    ...
    this.medicalLicenseRegExp =
      config.medicalLicenseRegExp ?? DEFAULT_MEDICAL_LICENSE_REGEXP;
    this.premiumLicenseRegExp =
      config.premiumLicenseRegExp ?? DEFAULT_PREMIUM_LICENSE_REGEXP;
  }

  parse(data: string) {
    const matches =
      this.medicalLicenseRegExp.exec(data) ??
    ...
    if (rawEndDay === undefined)
      throw new Error(
        `Can't find a license end day in '${data}', expected a 'ATÉ dd/mm/yy' or 'ATÉ dd/mm/yyyy'`,
      );
