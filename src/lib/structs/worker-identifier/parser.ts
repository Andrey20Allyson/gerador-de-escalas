...
export class WorkerIdentifierParser {
  parse(data: WorkerIdentifierParserData): WorkerIdentifier {
    ...
    const identifier = new WorkerIdentifier(+values[0]!, +values[1]!);
  ...
  private error(data: WorkerIdentifierParserData, complement?: string) {
    return new InvalidIdentifierError(
      `workerId of ${JSON.stringify(data.name ?? "Unknown")} : ${JSON.stringify(data.workerId)} don't is a valid worker id.${complement ? ` ${complement}` : ""}`,
    );
  }
}
