...
export class AssignmentRuleStack implements AssignmentRule {
  ...
  canAssign(worker: WorkerInfo, duty: ExtraDuty): boolean {
    return this.rules.every((rule) => rule.canAssign(worker, duty));
  }
  ...
  private _find(
    predicate: (rule: AssignmentRule) => boolean,
    root: AssignmentRule = this,
  ): AssignmentRule | null {
  ...
  find<T extends AssignmentRule>(
    predicate: (rule: AssignmentRule) => rule is T,
  ): T | null;
  ...
  static find<T extends AssignmentRule>(
    rule: AssignmentRule,
    predicate: (rule: AssignmentRule) => rule is T,
  ): T | null;
  static find(
