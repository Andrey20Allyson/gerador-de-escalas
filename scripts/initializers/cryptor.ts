...
import CryptorWithPassword from '../../src/app/auto-schedule/cryptor/cryptor-with-password';
...
export class CryptorProgram {
  ...
  static load(command: Command): CryptorProgram {
    ...
    if (typeof outputPath !== 'string') command.error(`Error: required option 'output' hasn't recived`);
    ...
    if (typeof password !== 'string') command.error(`Error: required option 'password' hasn't recived`);

    return new CryptorProgram(
      password,
      inputPath,
      outputPath
    );
